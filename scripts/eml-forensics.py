...
"""
import sys, os, re, base64, email, email.policy, hashlib, html as H, unicodedata, collections, argparse, binascii

ZW = {0x200B: "ZERO WIDTH SPACE", 0x200C: "ZERO WIDTH NON-JOINER",
      0x200D: "ZERO WIDTH JOINER", 0xFEFF: "ZERO WIDTH NO-BREAK SPACE",
...
def deobfuscate(src):
    ...
    t = re.sub(r'<span style="color:\s*(white|#fff{1,3}|rgb\(255,\s*255,\s*255\))"[^>]*>.*?</span>', "", t, flags=re.S | re.I)
    ...
    for raw in cands:
        ...
        for cand in (norm, norm + "=", norm + "=="):
            ...
            try:
                ...
                continue
            dec = dec.strip()
            if dec and sum(ch.isprintable() for ch in dec) / len(dec) > 0.9 and re.search(r"[A-Za-z0-9]", dec):
                out.append((raw, dec))
            break
    return out
...
def main():
    ...
    for part in msg.walk():
        ...
        if ct == "text/html" and html_body is None:
            ...
            txt_body = part.get_payload(decode=True).decode(part.get_content_charset() or "utf-8", "replace")

    written = [(os.path.basename(a.eml), sha256(a.eml), "oryginał wiadomości RFC 822 — DOWÓD GŁÓWNY, nie modyfikować"),
    ...
    if html_body is not None:
        ...
        written.append((os.path.basename(p), sha256(p), "część text/html po zdekodowaniu transfer-encoding"))
    ...
    W(f"Wygenerowano skryptem `eml-forensics.py`. SHA-256 pliku źródłowego: `{sha256(a.eml)}`\n")

    W("## 1. Identyfikacja\n")
    W("| Nagłówek | Wartość |")
    W("|---|---|")
    _seen_h = set()
    for k in ("Message-Id", "Date", "From", "To", "Cc", "Reply-To", "Return-Path",
              "Subject", "List-Unsubscribe", "X-Mailer", "User-Agent"):
        v = msg.get(k)
    ...
    W("")
    ips = collections.OrderedDict.fromkeys(re.findall(r"\[?((?:\d{1,3}\.){3}\d{1,3})\]?", " ".join(map(str, rec))))
    if ips:
    ...
    if auth:
        ...
        W("```")
        verdict = []
        if "dkim=pass" in auth: verdict.append("DKIM **pass**")
        if "dkim=fail" in auth: verdict.append("DKIM **fail**")
        if "spf=pass" in auth: verdict.append("SPF **pass**")
        if "spf=fail" in auth or "spf=softfail" in auth: verdict.append("SPF **fail/softfail**")
        if verdict:
            W("\n" + ", ".join(verdict) + ".")
    ...
    d_from, d_reply, d_ret = dom(msg.get("From")), dom(msg.get("Reply-To")), dom(msg.get("Return-Path"))
    links = collections.OrderedDict.fromkeys(re.findall(r'https?://([A-Za-z0-9.-]+)', src))
    W("| Rola | Domena |")
    W("|---|---|")
    ...
    if len(distinct) > 1:
        ...
        W(f"> Wykryto **{_n} {_f}**. Rozbieżność domeny nadawczej i domeny marki "
    ...
    if re.match(r"\s*(re|odp|fwd|fw)\s*:", subj, re.I) and not has_thread:
        W(f"> **Wątek sfingowany.** Temat `{subj}` sugeruje odpowiedź na wcześniejszą korespondencję, "
    ...
    for m in re.finditer(r"[?&](?:unsu|unsub|email|e|u|id)=([^\"'&\s>]+)", src + hdr, re.I):
    ...
    if n: rows.append(("Komentarze HTML wewnątrz wyrazów", n,
                       (re.search(r"\w<!--.*?-->\w", src, flags=re.S) or re.search(r"<!--.*?-->", src, flags=re.S)).group(0)[:70]))
    zwc = collections.Counter(ord(c) for c in src if ord(c) in ZW)
    for cp, cnt in zwc.items():
        ctx = ""
        m = re.search(r"\w" + re.escape(chr(cp)) + r"\w", src)
        if m: ctx = repr(m.group(0))
        rows.append((f"Znaki niedrukowalne U+{cp:04X} ({ZW[cp]})", cnt, ctx))
    ent = collections.Counter(re.findall(r"&#\d+;", src))
    if ent: rows.append(("Numeryczne encje HTML zamiast liter", sum(ent.values()),
                         " ".join(f"{k}×{v}" for k, v in ent.most_common(6))))
    sp = re.findall(r"<span>(.*?)</span>", src, flags=re.S)
    if sp: rows.append(("Puste znaczniki &lt;span&gt; rozbijające wyrazy", len(sp),
                        " · ".join(f"<span>{x[:14]}</span>" for x in sp[:3])))
    white = re.findall(r'<span style="color:\s*(?:white|#fff{1,3})"[^>]*>(.*?)</span>', src, flags=re.S | re.I)
    if white: rows.append(("Tekst niewidoczny (biały na białym) — hash busting", f"{sum(len(w) for w in white)} znaków",
                           white[0][:70].replace("\n", " ")))
    inv = re.findall(r'style="[^"]*(?:display:\s*none|font-size:\s*0|visibility:\s*hidden)[^"]*"', src, re.I)
    if inv: rows.append(("Elementy ukryte przez CSS (display:none / font-size:0)", len(inv), inv[0][:70]))
    if rows:
    ...
    dcc = msg.get("X-DCC--Metrics") or msg.get("X-Spam-Status") or msg.get("X-Spam-Flag")
    if dcc: W(f"Nagłówek filtra po stronie odbiorcy: `{dcc}`\n")
